...
internal class BukkitScopeHolder(
    ...
    private val supervisorScope: CoroutineScope by lazy {
        CoroutineScope(supervisorJob + plugin.mainDispatcher)
    }
    ...
    private fun getPlayerScope(player: Player): CoroutineScope {
        ...
        return playerScopes.getOrPut(player) {
            CoroutineScope(playerSupervisor + plugin.mainDispatcher)
        }
    }
...
public fun ScopeHolder.Companion.of(plugin: Plugin): ScopeHolder = BukkitScopeHolder(plugin)